        ...
        static void generaSchedina()
        {
            ...
            for(int i = 0; i<3; i++) //ciclo una volta per riga
            {
                ...
                for (int j = 0; j <9; j++) //riempio la scheda con i numeri
                {
                    ...
                    {
                        int estratto;
                        if (j == 0) estratto = rnd.Next(1, 10);                         //eccezione: lo 0 non c'è
                        else if (j == 8) estratto = rnd.Next(j * 10, (j+1) * 10 + 1);   //eccezione: c'è anche il 90
                        else estratto = rnd.Next(j * 10, (j + 1) * 10);                 //funzionamento normale: da decina fino alla prossima decina non compresa
